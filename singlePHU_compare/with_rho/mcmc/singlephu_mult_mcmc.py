...
x_MLE_low = [ 0.15091599 - bval, -0.12033003 - bval,  0.08601739 - bval, -0.09082558 - bval, 0.03765093 - bval,  0.08950242 - bval]

x_MLE_up = [ 0.15091599 + bval, -0.12033003 + bval,  0.08601739 + bval, -0.09082558 + bval, 0.03765093 + bval, 0.08950242 + bval ]
...
mylabel = [r'$a_{0}^{1}$',r'$a_{1}^{1}$', r'$a_{2}^{1}$',r'$a_{3}^{1}$', r'$a_{4}^{1}$', r'$a_{5}^{1}$']
...
file = np.genfromtxt('./toronto_synthetic_data_mult.csv', delimiter=',')
...
####### RECALIBRATED FOR RHO = 0.25 TO GET INFECTED MATCHING MOH ##########
        ...
        if( kk%ndiv == 0 ):
            ...
            if(tstart !=0):
                ...
                if(idxmoh >=tstart and idxmoh<tlim):
                    ...
                    multiplier = (1/(np.sqrt(2*np.pi)*sigma * rho * I[kk,0]))


                    err = (I_synthetic[idxdata,0] - rho * I[kk,0])**2

                    # log likelihood
                    loglik = loglik  + np.log(multiplier) - (err/(2*(sigma * rho * I[kk,0])**2)) 
                ...
                err = (I_synthetic[idxmoh,0] - rho * I[kk,0])**2
